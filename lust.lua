...
function tree_dump_helper(t, p, strlimit)
	if type(t) == "table" then
		...
		for k, v in pairs(t) do	
			...
			terms[#terms+1] = format("[%s] = %s,", key, tree_dump_helper(v, p1, strlimit-2))
		end
		return format("%s\n%s}", concat(terms, "\n"..p1), p)
...
local grammar = P{
	...
	Rule(V"indent"^-1 * (V"something" + V"anything")^0, "main"),
	...
	something 			= (Rule(newline, "newline") * V"indent")
						+ V"insert",
	
	indent				= Rule(C(space^1), "indent"),
	...
	inline_term			= V"something" 
						+ Rule(C((P(1) - (P"}}" + V"something"))^1), "inline_text"),
	inline				= P"{{" 
						* Rule(
							V"inline_term"^0, 
							"inline"
...
function action:main(node, out)
	...
	out("local result = concat(out):gsub('\\n', '\\n'..indent)")
	out("return result")
end
...
function action:newline(node, out)
	out:write("out[#out+1] = '\\n'")
end

function action:indent(node, out)
	local s = node[1]
	out:comment("new indent = indent .. %q", s)
	--out:format('print(format("indent %%q", %q))', s)
	self.indent = s
	out:format("out[#out+1] = '%s'", s)
end
...
function action:loop_helper(body, out, it, start, len, terms, mt, sep, parent)
	...
	if sep then
		insep = gensym("sep")
		out:format("local %s = gsub(%s, '\\n', '\\n'..%q)", insep, sep, self.indent)
	end
	...
	out:comment("loop body end, indent %q", indent)
...
function action:apply_helper(tmp, env, out)
	...
	out:format("if %s then", tmp)
		:push()
	out:format("%s = %s(%s, indent..%q)", name, tmp, env, self.indent)
	out:pop()
		:write("end")
	return name
end
...
function action:apply_inline_helper(body, env, out)
	...
	out:format("local %s", name)
		...
		:write("local out = {}")
		--:write("local write = function(s) out[#out+1] = s end")
	local rule = self:dispatch(body, out)
	out:format("%s = concat(out):gsub('\\n', '\\n'..%q)", name, self.indent)
		:pop()
		:write("end")
	return name
end
...
function gen:dispatch(node, out)
	if type(node) == "table" then
		...
		if rule and action[rule] then	
			...
			return action[rule](self, node, out)
		else
			tree_dump(node)
...
function gen:reify(name)
	...
	if name == 1 then
		...
		rulename = "template_" .. name:gsub("(%.)", "_")
	end
	
	local out = writer.create()
	-- header:
	out:comment("rule %s:", name) 
	out:format("local function %s(env, indent)", rulename) 
		:push()
	...
	if type(name) == "number" then
		out:format("rules[%s] = %s", name, rulename)
	else
		out:format("rules[%q] = %s", name, rulename)
	end
